  ...
  try {
    ...
    if (parseError) {
      ...
      return {
        ...
        line: lineMatch ? Number.parseInt(lineMatch[1]) : 1,
      };
    }
    return null;
  } catch (e: any) {
...
export const validateWithLibxml = async (
  xml: string,
  xsd: string
  ...
  try {
    ...
    try {
      ...
      return { valid: false, errors: [{ line: 1, message: `XSD Parse Error: ${e.message}` }] };
    }
    ...
    try {
      ...
      return { valid: false, errors: [{ line: 1, message: `Invalid XSD Schema: ${e.message}` }] };
    }
    ...
    try {
      ...
      return { valid: false, errors: [{ line: 1, message: `XML Parse Error: ${e.message}` }] };
    }
    ...
    try {
      ...
      if (e instanceof XmlValidateError) {
        const errors = e.details.map((d: any) => ({
          line: d.line || 1,
    ...
    console.error("Libxml2 validation error:", e);
    return { valid: false, errors: [{ line: 1, message: `Validation Error: ${e.message}` }] };
  } finally {
...
export const validateAgainstSchema = async (
  ...
  if (useLibxml) {
    return validateWithLibxml(xml, xsd);
  }

  // Legacy (Custom) Validation Logic
  if (!(xml && xsd)) return { valid: true, errors: [] };
  ...
  if (xsdDoc.querySelector("parsererror")) {
    return {
      valid: false,
      errors: [{ line: 1, message: "Invalid XSD schema format" }],
    };
  }
  ...
  if (xmlDoc.querySelector("parsererror")) {
    return {
      valid: false,
      errors: [{ line: 1, message: "XML is not well-formed" }],
    };
  }
  ...
  xsdDoc
    .querySelectorAll('[localName="complexType"], complexType')
    .forEach((ct) => {
      ...
      if (name) {
        ...
        ct.querySelectorAll('[localName="element"], element').forEach((e) => {
          elements.push({
            ...
            minOccurs: Number.parseInt(e.getAttribute("minOccurs") || "1"),
            maxOccurs:
              e.getAttribute("maxOccurs") === "unbounded"
                ? Number.POSITIVE_INFINITY
                : Number.parseInt(e.getAttribute("maxOccurs") || "1"),
          });
        });
        ct.querySelectorAll('[localName="attribute"], attribute').forEach(
          (a) => {
            attrs.push({
  ...
  if (elementDefs.size > 0 && !elementDefs.has(rootName)) {
    ...
    errors.push({
      line: 1,
      message: `Root element <${rootName}> is not defined in schema. Expected: ${validRoots}`,
    });
  }
    ...
    if (def && def.type && complexTypes.has(def.type)) {
      ...
      typeDef.attrs.forEach((attr: any) => {
        if (attr.required && !xmlEl.hasAttribute(attr.name)) {
          ...
          errors.push({
            line: lineNum,
            message: `Missing required attribute "${attr.name}" on <${elName}>`,
          });
        }
      });
      ...
      typeDef.elements.forEach((childDef: any) => {
        if (childDef.minOccurs > 0) {
          const count = xmlEl.querySelectorAll(
            ":scope > " + childDef.name
          ).length;
          if (count < childDef.minOccurs) {
            const lineNum = getLineNumber(
              xml,
              xmlEl.outerHTML.substring(0, 50)
            );
            errors.push({
              line: lineNum,
              message: `Element <${elName}> requires at least ${childDef.minOccurs} <${childDef.name}> child element(s)`,
            });
          }
        }
      });
...
export const highlightXML = (
  xml: string,
  colors: EditorColors = defaultColors
): string => {
  if (!xml) return "";
  ...
  const c = (k: string, t: string) =>
    '<span style="color:' + k + '">' + e(t) + "</span>";
  while (i < n) {
    if (xml.slice(i, i + 4) === "<!--") {
      let x = xml.indexOf("-->", i);
      x = x < 0 ? n : x + 3;
      ...
      if (cl) j++;
      let t = "";
      while (j < n && /[\w:-]/.test(xml[j])) {
        t += xml[j++];
      }
      r += c(col.tagBracket, cl ? "</" : "<");
      if (t) r += c(col.tagName, t);
      while (
        j < n &&
        xml[j] !== ">" &&
        !(xml[j] === "/" && xml[j + 1] === ">")
      ...
      r += '<span style="color:' + col.text + '">' + e(t) + "</span>";
    }
  }
  return r;
};
